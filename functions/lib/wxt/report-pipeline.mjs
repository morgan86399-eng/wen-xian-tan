...
export function buildActionsPrompt(themeId, answers) {
  ...
  return [
    ...
    '時間要放在句子中間，嚴禁用時間詞當作句子的開頭，三條也不要都指向同一天。',
    '三條的句型結構不可以相同。',
...
export function fallbackActions(themeId, answers) {
  ...
  return [
    `把「${short}」在今晚睡前寫成三句話，明天早上再讀一次，圈出最先要處理的那一句`,
    '找一位信任的人在這週三以前當面講完這件事，只講事實與自己的感受，先不要求對方給答案',
    '挑一個週末的下午，把手上的實際數字與狀況列成清單，看清楚缺口落在哪一段'
  ];
}
...
async function repairActions(env, { themeId, answers, report }) {
  const systemPrompt = '你是Zenasker的解讀顧問，只負責產出可執行的行動建議，不寫其他內容。';
