...
await check('命中百分比與 W 系列禁詞', async () => {
  ...
  assert.ok(hits.includes('卡住'));
  assert.ok(hits.some((h) => /%/.test(h)));
  const cleaned = replaceForbidden('這段很爛，卡住了 50%');
  assert.ok(!cleaned.includes('爛'));
});

await check('禁詞替換不可以把正常詞改成亂碼', async () => {
  // 單字全域替換曾經把這幾句改壞，而這是直接送到付費使用者眼前的正文
  assert.equal(replaceForbidden('請檢視信用卡帳單明細'), '請檢視信用卡帳單明細');
  assert.equal(replaceForbidden('這張卡片先剪掉'), '這張卡片先剪掉');
  assert.equal(replaceForbidden('最容易引爆沉默戰爭的時刻'), '最容易引發沉默戰爭的時刻');
  assert.equal(replaceForbidden('把情緒垃圾桶往外放'), '把情緒回收桶往外放');
  assert.equal(replaceForbidden('收拾這個爛攤子'), '收拾這個難以收拾的局面');
  // 合法用法不該被判違規
  assert.equal(scanForbidden('請檢視信用卡帳單明細').length, 0);
  assert.equal(scanForbidden('走路回家時掉眼淚').length, 0);
  // 該擋的還是要擋
  assert.ok(scanForbidden('資金週轉卡住').includes('卡住'));
  assert.ok(scanForbidden('情緒爆發').includes('爆發'));
});
